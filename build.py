...
$("#budLock").onclick=()=>{budUnlocked=false;budAdmin=false;$("#budAdmin").textContent="✏️ 금액·내용 편집";$("#budAdmin").classList.remove("on");$("#budRestore").style.display="none";budView();};
...
const tb=$("#tabbar");
TABS.forEach(([id,label,cnt])=>{const b=ce("button");b.dataset.t=id;b.innerHTML=label+(cnt?` <span class="b">${cnt()}</span>`:"");if(id==="overview")b.classList.add("on");b.onclick=()=>goTab(id);tb.appendChild(b);});
